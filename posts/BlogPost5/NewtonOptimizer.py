...
# IMPLEMENTING LOGISTIC REGRESSION WITH NEWTONS METHOD
...
class LogisticRegression(LinearModel):
    ...
    def logistic_hessian(self, X, y):
        ...
        s_i = self.score(X)
        sigmoid_si = torch.sigmoid(s_i)
        D_w = sigmoid_si * (1 - sigmoid_si)
        matrix = X.T@(torch.diag(D_w))@X
...
class NewtonOptimizer(LogisticRegression):
    ...
    def step(self, X, y, alpha):
        ...
        hessian_matrix = self.model.logistic_hessian(X, y)
        self.model.w = self.model.w - (alpha * torch.inverse(hessian_matrix)) @ gradient
